...
CHANGED NOT WORKING
BACKUP IN DESKTOP FILDER Alex Backups
...
public class Antharas extends Quest implements Runnable {
	...
	private void init() {
		...
		if (FWA_OLDANTHARAS || status == WAITING) {
			final StatsSet info = GrandBossManager.getInstance().getStatsSet(ANTHARASNORMALID);
			...
			if (status == DEAD && respawnTime <= System.currentTimeMillis()) {
				...
				GrandBossManager.getInstance().setBossStatus(ANTHARASNORMALID, FIGHTING);
				status = FIGHTING;
				...
				_antharas = (L2GrandBossInstance) addSpawn(ANTHARASNORMALID, loc_x, loc_y, loc_z, heading, false, 0);
				...
				ThreadPoolManager.getInstance().scheduleGeneral(new UnlockAntharas(ANTHARASNORMALID), respawnTime - System.currentTimeMillis());
	...
	public void setAntharasSpawnTask() {
		if (_monsterSpawnTask == null) {
			synchronized (this) {
				if (_monsterSpawnTask == null) {
					GrandBossManager.getInstance().setBossStatus(ANTHARASNORMALID, WAITING);
	...
	private static class UnlockAntharas implements Runnable {
		...
		@Override
		public void run() {
			GrandBossManager.getInstance().setBossStatus(_bossId, FIGHTING);
